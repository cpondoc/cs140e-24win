...
{
    // cycle_cnt_read -- defined in cycle-count.h
    uint32_t s;
    while ((s = cycle_cnt_read()) < start_cycle);

    // Then, run function until it is there
    while ((s = cycle_cnt_read()) < start_cycle + ncycles) {
        if (fn) {
            fn();
        }
    }
