...
void notmain(void) { 
    ...
    // Tagging the heap
    pin_mmu_sec(idx++, OneMB, OneMB, heap_pin);        // tlb 4
    ...
    // Set the permissions by doing something like
    // (DOM_client << dom_kern * 2) | (DOM_client << dom_heap * 2)
    // Specifically, do like dom no access?
    // Then do a get or something
    /*
