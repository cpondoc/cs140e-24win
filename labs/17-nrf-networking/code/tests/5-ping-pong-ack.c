...
enum { ntrial = 1000, timeout_usec = 4000 };
...
one_way_noack(nrf_t *server, nrf_t *client, int verbose_p) {
    ...
    unsigned server_addr = server->rxaddr;
    ...
    for(unsigned i = 0; i < ntrial; i++) {
        ...
        uint32_t old = -1;
        uint32_t x;
        if(nrf_read_exact_timeout(server, &x, 4, timeout_usec) == 4) {
            ...
            /*if (old == -1) {
                ...
                nrf_output("new packet: %d previous packet: %d\n", x,old);
            }*/
            if(x != i) {
                nrf_output("lost/dup packet: received %d (expected=%d, difference=%d)\n", x,i, x-i);
                client->tot_lost++;
            }
            npackets++;
        } else {
